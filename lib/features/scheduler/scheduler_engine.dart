...
class SchedulerEngine {
  ...
  SchedulerEngine({
    ...
    required this._schedulerRepository,
    required this._jobManager,
    required this._backupEngine,
  });
    ...
    try {
      ...
      for (final folder in folders) {
        ...
        if (isAvailable) {
          ...
          // If we had paused/failed jobs because of destination unavailability, they can now be resumed.
  ...
  String _getQueueTypeForSchedule(ScheduleConfig schedule) {
    // Determine priority based on type: Real-time is normal, custom/manual is priority, background trigger is background
