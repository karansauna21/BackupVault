...
class JobManager extends Notifier<List<ScheduledBackupJob>> {
  ...
  List<ScheduledBackupJob> build() {
    // Initialized lazily. State starts empty.
    return [];
  }
  ...
  Future<void> queueJob({
    ...
    // Avoid duplicates in active queues
  ...
  Future<void> _processQueue() async {
    ...
    while (true) {
      // Find next pending job prioritizing 'priority' queue
      final pendingJobs = state.where((j) => j.status == 'pending').toList();
      if (pendingJobs.isEmpty) break;

      // Sort: 'priority' first, then 'normal', then 'background', then 'retry'
  ...
  Future<void> _runFolderBackup(ScheduledBackupJob job) async {
    // Check if job is still pending before we start
    ...
    try {
      ...
      // Trigger standard folder backup in the BackupEngine
      await _backupEngine.backupFolder(folder);

      // Wait until all file-level jobs for this folder are done in the main queue manager
  ...
  /// Wait for the main queue manager to finish processing all file-level jobs for this folder
  ...
  /// Update folder-level progress by looking at file-level progress in the core copyQueueProvider
    ...
    final updatedList = state.map((job) {
      if (job.status == 'running') {
        ...
        if (folderJobs.isNotEmpty) {
          ...
          // Estimate folder progress as completed/failed files vs total files, plus current progress of running files
  ...
  /// Save queue to support folder configuration
  ...
  /// Clear the completed and failed history from the memory list
